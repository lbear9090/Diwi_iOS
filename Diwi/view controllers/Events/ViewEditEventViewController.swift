...
class ViewEditEventViewController: UIViewController, UITextFieldDelegate {
    ...
    var viewModel: ViewEditEventViewModel!
...
// MARK: - Setup bindings
extension ViewEditEventViewController {
    private func setupBindings() {
        ...
        viewModel.note
            .asObservable()
            .bind(to: note.rx.text)
            .disposed(by: disposeBag)
        
        viewModel.fullAddress
            .asObservable()
            .skip(1)
            .bind(to: locationView.textField.rx.text)
            .disposed(by: disposeBag)
        ...
        note.rx.didBeginEditing.bind { [unowned self] in
            ...
            if self.note.text == TextContent.Labels.addANote {
                self.note.text = nil
            }
        }.disposed(by: disposeBag)
        ...
        note.rx.didEndEditing.bind { [unowned self] in
            if self.note.text.isEmpty {
                ...
                self.note.textColor = UIColor.Diwi.fadedGray
            }
        }.disposed(by: disposeBag)
...
extension ViewEditEventViewController {
    ...
    private func setupNav() {
        ...
        navTitle.text = TextContent.Labels.myEvent
    ...
    private func setupWhatDidIWearView() {
        ...
        NSLayoutConstraint.activate([
           ...
           whatDidIWearView.heightAnchor.constraint(equalToConstant: 28),
        ])
    }
    ...
    private func setupWhoWasThere() {
        ...
        NSLayoutConstraint.activate([
           ...
           whoWasThereView.heightAnchor.constraint(equalToConstant: 24),
        ])
    }
...
extension ViewEditEventViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
